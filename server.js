  ...
  try {
    ...
    const [comments] = await pool.query(
      `SELECT tc.*, u.full_name as user_name, u.role
       ...
       WHERE tc.ticket_id=? ORDER BY tc.created_at ASC`, [req.params.id]);
...
app.post('/api/tickets/:id/comments', auth, async (req, res) => {
  ...
  if (!comment && !command_output) return res.status(400).json({ error: 'Comment or command output required' });
  try {
    await pool.query(
      'INSERT INTO ticket_comments (ticket_id, user_id, comment, command_output) VALUES (?,?,?,?)',
